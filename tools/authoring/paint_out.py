...
PROMPT = """IMAGE 1 is the front elevation of a {asset}, drawn flat and
straight on.
IMAGE 2 is the same picture with some areas painted over in flat BRIGHT MAGENTA.

Redraw IMAGE 2 with every magenta area filled in, and nothing else changed.

Under each magenta area a fitting was removed. Draw what is BEHIND it: the same
surface as the picture immediately AROUND that magenta area, continuing
straight through it -- the same colour, the same brightness, the same grain,
the same streaks and grime running the same way. A magenta patch surrounded by
pale glass fills with pale glass. One surrounded by dark metal fills with dark
metal. Match each patch to ITS OWN surroundings, not to the rest of the {asset}.

  - EVERYTHING THAT IS NOT MAGENTA MUST COME BACK IDENTICAL. Same size, same
    position, same angle, same outline, same palette, same lighting. Do not
    remove, move, clean up, brighten or redraw anything outside the magenta.
    In particular do not empty out a window, a case or a shelf that is not
    itself painted magenta.
  - NO MAGENTA MAY REMAIN, and no blank, flat patch, white area, shadow or
    outline may be left in its place. Draw the material.
  - Do not draw the removed fitting back, even faintly.

Hand-painted PlayStation-era look, same resolution and same style as image 1."""

# WHY THE HOLES ARE SHOWN AND NOT LISTED. This asked for "the prop with its
# doors, panels, buttons, slots, grilles, signs, screens, shelves, handles and
# labels taken off", which is a description of the holes rather than the holes,
# and it is the mistake detail_sheet already recorded: a model given a
# description draws what a thing of that name usually looks like.
#
# On the vending machine it read the list and emptied the entire display case
# -- glass, shelves and all -- and painted a dark rusty back wall behind where
# the window had been. Defensible, and useless: only the product HOLES are
# composited, so each one came back dark against pale glass that had not moved,
# and the colour check refused every product on the machine (distances 42 to
# 74 against a bar of 46; grain was never the problem). Ten holes fell through
# to the arithmetic tile, which is the fault this tool exists to remove.
#
# Painting the holes flat magenta says exactly what the composite is going to
# take and nothing else, in the one language that cannot be misread as a
# category. It is the same bargain as everywhere else here: the model is shown
# the thing rather than told about it.
MAGENTA = (255, 0, 255)
...
def main():
    ...
    # is shown a magenta mask covering exactly the outstanding holes, which is
    ...
    for attempt in range(max(1, args.tries)):
        if not todo:
            break
        # the holes still outstanding, as flat magenta over the elevation
        marked = src.copy()
        mk = marked.load()
        want = [[False] * H for _ in range(W)]
        for p in todo:
            x0, y0, x1, y1 = p["px"]
            for x in range(max(0, x0), min(W, x1)):
                for y in range(max(0, y0), min(H, y1)):
        ...
        try:
            ...
            print("  plate unreadable -- arithmetic fill stands for the rest")
            break
        ...
        for p in todo:
            ...
            # the material AROUND this hole, which is what the fill has to
            # match -- not the prop's modal colour, which is a different
            # question and the one the arithmetic fill answered wrongly
            ...
            # AND NO MAGENTA MAY SURVIVE. A model that leaves the mask where it
            # found it passes grain easily and would paint a fitting-shaped
            # magenta rectangle into the background.
            ...
            for x in range(max(0, x0), min(W, x1)):
                for y in range(max(0, y0), min(H, y1)):
                    if hole[x][y]:
                        ...
                        mp[x, y] = 255
            kept += 1
        if len(still) == len(todo) and attempt:
            todo = still
            break                     # a second drawing that gained nothing
        todo = still
    ...
    print(f"  {kept} fitting(s) painted out by the model, {len(todo)} left to "
