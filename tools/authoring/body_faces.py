...
from layer_build import sheet_mask, silhouette  # noqa: E402
...
def cut_to_silhouette(im):
    ...
    CUT WHAT IS SHEET, NOT WHAT IS PALE -- see layer_build.sheet_mask(). A dark
    trim strip down a machine's flank is not prop-coloured, and cut, it became a
    slit running the prop's whole height that the renderer alpha-tested into
    daylight. Six props in the corpus had one and none was visible from the
    front.
    """
    out = im.convert("RGBA")
    W, H = out.size
    gone = sheet_mask(silhouette(im, erode=0), W, H)
    px = out.load()
    n = 0
    for x in range(W):
        for y in range(H):
