...
| the two judges | per round | **Nothing grades the grader -- and now there is a number.** On one prop, same renders and schema: paid 5 judged / 5 FAIL, local qwen2.5vl:7b 5 judged / **0 FAIL**. Three of the five local verdicts describe the defect and pass it ("the part repeats in a uniform band, maintaining the proportions"). A loop run to convergence on this judge would call a cabinet with five visible faults shippable. **Nothing grades the grader.** A weak model here degrades silently. Note this is already true of the paid judges: `judge_bench` puts both at chance on the one fault class it can score, and CLAUDE.md records both scoring a cabinet with five stacked marquees level with the corrected one beside it. Running them locally is not obviously worse — it is the same unmeasured channel. |
